...
def write_cells_def(topmodule_file, top_module_file_out, macros_locations, def1_data, def2_data):
  ...
  with open(topmodule_file, 'r') as in_f, open(top_module_file_out, 'w') as out_f, open(macros_locations, 'w') as macros_out_f:
    ...
    for line in in_f:
      ...
      if parse_components == 1:
        ...
        if def2_data["prefix"] == prefix:
          name = parts[1].split("/", 1)[1].replace("\\", "")
          new_x = float(def2_data["components"][name]["x"]) + (float(def2_data["xoffset"]) * float(def2_data["db_units"]))
          new_y = float(def2_data["components"][name]["y"]) + (float(def2_data["yoffset"]) * float(def2_data["db_units"]))

          orientation = def2_data["components"][name]["orientation"]

          if "fakeram" in parts[2]:
            new_x_cart = new_x / float(def2_data["db_units"])
            new_y_cart = new_y / float(def2_data["db_units"])
...
if __name__ == '__main__':
  if len(sys.argv) != 12:
    print("Usage: python parse_def.py <top_module> <Core1> <Core2> <def1_module_name> <def2_module_name> <def1_llx> <def1_lly> <def2_llx> <def2_lly>")
    sys.exit(1)
  ...
  Core1 = sys.argv[2]
  Core2 = sys.argv[3]
  def1_module_name = sys.argv[4]
  def2_module_name = sys.argv[5]
  def1_llx = sys.argv[6]
  def1_lly = sys.argv[7]
  def2_llx = sys.argv[8]
  def2_lly = sys.argv[9]
  top_module_out = sys.argv[10]
  macros_locations_out = sys.argv[11]
  ...
  snapped_def2_llx = 0.0
  snapped_def2_lly = 0.0
  if is_exact_multiplicand(float(def2_llx), sitex):
    snapped_def2_llx = float(def2_llx)
  else:
    snapped_def2_llx = (sitex * (float(def2_llx) / sitex)) + sitex
  if is_exact_multiplicand(float(def2_lly), sitey):
    snapped_def2_lly = float(def2_lly)
  else:
    snapped_def2_lly = (sitey * (float(def2_lly) / sitey)) + sitey/2


  print(f"Snapped coordinates for core 1: {snapped_def1_llx} {snapped_def1_lly}")
  print(f"Snapped coordinates for core 2: {snapped_def2_llx} {snapped_def2_lly}")

  def1_data = parse_def_file(Core1, def1_module_name, snapped_def1_llx, snapped_def1_lly)
  def2_data = parse_def_file(Core2, def2_module_name, snapped_def2_llx, snapped_def2_lly)

  write_cells_def(top_module, top_module_out, macros_locations_out, def1_data, def2_data)